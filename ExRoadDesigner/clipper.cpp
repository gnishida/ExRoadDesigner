...
// Note:
//		delta>0
void ClipperOffset::Execute(Paths& solution, std::vector<double> delta){
  solution.clear();
  FixOrientations();
  DoOffset(delta);
...
void ClipperOffset::DoOffset(double delta)
{
	...
	//printf("m_polyNodes.ChildCount() %d\n",m_polyNodes.ChildCount());
	for (int i = 0; i < m_polyNodes.ChildCount(); i++)
	{
		PolyNode& node = *m_polyNodes.Childs[i];
...
/*// NOTE: 
//		No poly len=1
//		Just etClosedPolygon
void ClipperOffset::DoOffset(std::vector<double> delta)
{
	m_destPolys.clear();
	m_delta = delta;
	...
	double y;
	if (ArcTolerance <= 0.0) y = def_arc_tolerance;
	else if (ArcTolerance > std::fabs(delta[0]) * def_arc_tolerance) 
		y = std::fabs(delta[0]) * def_arc_tolerance;
	else y = ArcTolerance;
	//see offset_triginometry2.svg in the documentation folder ...
	double steps = pi / std::acos(1 - y / std::fabs(delta[0]));
	if (steps > std::fabs(delta[0]) * pi) 
		steps = std::fabs(delta[0]) * pi;  //ie excessive precision check
	m_sin = std::sin(two_pi / steps);
	m_cos = std::cos(two_pi / steps);
	m_StepsPerRad = steps / two_pi;
	if (delta[0] < 0.0) m_sin = -m_sin;
	...
	for (int i = 0; i < m_polyNodes.ChildCount(); i++)
	{
		...
		int len = (int)m_srcPoly.size();
		if (len == 0 || (delta[0] <= 0 && (len < 3 || node.m_endtype != etClosedPolygon)))
			continue;
