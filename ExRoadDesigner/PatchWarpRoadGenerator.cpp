...
void PatchWarpRoadGenerator::generateRoadNetwork() {
	srand(12345);
 ...
 * シード頂点を生成する。
 */
void PatchWarpRoadGenerator::generateAvenueSeeds(std::list<RoadVertexDesc>& seeds) {
	seeds.clear();
 ...
 */
bool PatchWarpRoadGenerator::addAvenueSeed(ExFeature &f, const QVector2D &pt, const QVector2D &ex_pt, int group_id, int ex_id, float angle, std::list<RoadVertexDesc>& seeds) {
 ...
 */
void PatchWarpRoadGenerator::generateStreetSeeds(std::list<RoadVertexDesc> &seeds) {
	seeds.clear();
 ...
 * Exampleベースで生成する。
 */
void PatchWarpRoadGenerator::attemptExpansion(int roadType, RoadVertexDesc srcDesc, ExFeature &f, std::vector<RoadEdgeDescs> &shapes, std::list<RoadVertexDesc> &seeds) {
...
void PatchWarpRoadGenerator::buildReplacementGraphByExample(int roadType, RoadGraph &replacementGraph, RoadVertexDesc srcDesc, RoadGraph &exRoads, RoadVertexDesc ex_srcDesc, float angle, RoadEdgeDescs &shape) {
	...
	// add vertices of the interesting shape
	{
		...
		for (QHash<RoadVertexDesc, bool>::iterator it = vertices.begin(); it != vertices.end(); ++it) {
			...
			replacementGraph.graph[v_desc]->properties["generation_type"] = "example";
			if (roadType == RoadEdge::TYPE_AVENUE) {
	...
	// interesting shapeのエッジを追加
	{
		for (int i = 0; i < shape.size(); ++i) {
			...
			replacementGraph.graph[e_desc]->properties["generation_type"] = "example";
...
void PatchWarpRoadGenerator::rewrite(int roadType, RoadVertexDesc srcDesc, RoadGraph &replacementGraph, std::list<RoadVertexDesc>& seeds) {
	QMap<RoadVertexDesc, RoadVertexDesc> conv;
...
bool PatchWarpRoadGenerator::isValidRule(RoadVertexDesc srcDesc, RoadGraph &replacementGraph) {
	RoadEdgeIter ei, eend;
 ...
 */
void PatchWarpRoadGenerator::attemptExpansion2(int roadType, RoadVertexDesc srcDesc, ExFeature &f, std::vector<RoadEdgeDescs> &shapes, std::list<RoadVertexDesc> &seeds) {
	float snapThreshold;
 ...
 */
bool PatchWarpRoadGenerator::growRoadSegment(int roadType, RoadVertexDesc srcDesc, ExFeature &f, const Polyline2D &polyline, int lanes, RoadVertexDesc next_ex_v_desc, bool byExample, float snapFactor, float angleTolerance, std::list<RoadVertexDesc> &seeds) {
	...
	if (!snapDone && RoadGeneratorHelper::getCloseVertex(roads, new_edge->polyline.last(), byExample, roads.graph[srcDesc]->properties["group_id"].toInt(), new_edge->polyline.length() * snapFactor, srcDesc, tgtDesc)) {
		snapTried = true;
	...
	if (!snapDone && RoadGeneratorHelper::getCloseEdge(roads, new_edge->polyline.last(), byExample, roads.graph[srcDesc]->properties["group_id"].toInt(), new_edge->polyline.length() * snapFactor, srcDesc, closestEdge, intPoint)) {
		snapTried = true;
 ...
 * PMに従って、カーネルを合成する
 */
std::pair<int, RoadVertexDesc> PatchWarpRoadGenerator::synthesizeItem(int roadType, RoadVertexDesc v_desc, std::vector<RoadEdgeDescs> &shapes, std::vector<RoadEdgePtr> &edges, float &rotation_angle) {
 ...
 */
/*
void PatchWarpRoadGenerator::replaceEdgeByExample(ExFeature &f, int roadType, RoadEdgePtr edge) {
	...
	for (boost::tie(ei, eend) = boost::edges(f.reducedRoads(roadType).graph); ei != eend; ++ei) {
		...
		if (f.reducedRoads(roadType).graph[*ei]->properties.contains("circle_id")) continue;
