...
void PatchWarp2RoadGenerator::generateRoadNetwork() {
	srand(12345);
		...
		std::vector<std::vector<RoadEdgeDescs> > shapes;
		{
			...
			for (int i = 0; i < features.size(); ++i) {
				...
				if (i == 0) {
					...
					shapes[i] = ShapeDetector::detect(features[i].reducedRoads(RoadEdge::TYPE_AVENUE), 0.02, G::getFloat("avenuePatchDistance"));
				}
				*/
	...
	if (G::getBool("generateLocalStreets")) {
		...
		std::vector<std::vector<RoadEdgeDescs> > shapes;
		{
			...
			for (int i = 0; i < features.size(); ++i) {
				...
				if (i == 0) {
					...
					shapes[i] = ShapeDetector::detect(features[i].reducedRoads(RoadEdge::TYPE_STREET), 0.02, G::getFloat("streetPatchDistance"));
				}
				*/
		...
		for (i = 0; !seeds.empty() && i < G::getInt("numStreetIterations"); ++i) {
			...
			int ex_id = roads.graph[desc]->properties["ex_id"].toInt();
			if (roads.graph[desc]->properties["generation_type"] == "example") {
				if (!attemptExpansion(RoadEdge::TYPE_STREET, desc, features[ex_id], shapes[ex_id], seeds)) {
 ...
 * シード頂点を生成する。
 */
void PatchWarp2RoadGenerator::generateAvenueSeeds(std::list<RoadVertexDesc>& seeds) {
	seeds.clear();
 ...
 */
bool PatchWarp2RoadGenerator::addAvenueSeed(ExFeature &f, const QVector2D &pt, const QVector2D &ex_pt, int group_id, int ex_id, float angle, std::list<RoadVertexDesc>& seeds) {
 ...
 */
void PatchWarp2RoadGenerator::generateStreetSeeds(std::list<RoadVertexDesc> &seeds) {
	seeds.clear();
 ...
 * Exampleベースで生成する。
 */
bool PatchWarp2RoadGenerator::attemptExpansion(int roadType, RoadVertexDesc srcDesc, ExFeature &f, std::vector<RoadEdgeDescs> &shapes, std::list<RoadVertexDesc> &seeds) {
...
void PatchWarp2RoadGenerator::buildReplacementGraphByExample(int roadType, RoadGraph &replacementGraph, RoadVertexDesc srcDesc, RoadGraph &exRoads, RoadVertexDesc ex_srcDesc, float angle, RoadEdgeDescs &shape) {
...
void PatchWarp2RoadGenerator::rewrite(int roadType, RoadVertexDesc srcDesc, RoadGraph &replacementGraph, std::list<RoadVertexDesc>& seeds) {
	QMap<RoadVertexDesc, RoadVertexDesc> conv;
...
bool PatchWarp2RoadGenerator::isValidRule(RoadVertexDesc srcDesc, RoadGraph &replacementGraph) {
	RoadEdgeIter ei, eend;
 ...
 */
void PatchWarp2RoadGenerator::attemptExpansion2(int roadType, RoadVertexDesc srcDesc, ExFeature &f, std::vector<RoadEdgeDescs> &shapes, std::list<RoadVertexDesc> &seeds) {
	float snapThreshold;
 ...
 */
bool PatchWarp2RoadGenerator::growRoadSegment(int roadType, RoadVertexDesc srcDesc, ExFeature &f, const Polyline2D &polyline, int lanes, RoadVertexDesc next_ex_v_desc, bool byExample, float snapFactor, float angleTolerance, std::list<RoadVertexDesc> &seeds) {
 ...
 * PMに従って、カーネルを合成する
 */
std::pair<int, RoadVertexDesc> PatchWarp2RoadGenerator::synthesizeItem(int roadType, RoadVertexDesc v_desc, std::vector<RoadEdgeDescs> &shapes, std::vector<RoadEdgePtr> &edges, float &rotation_angle) {
 ...
 */
/*
void PatchWarp2RoadGenerator::replaceEdgeByExample(ExFeature &f, int roadType, RoadEdgePtr edge) {
